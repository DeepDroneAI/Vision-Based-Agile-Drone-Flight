...
class PoseSampler:
    ...
    def run_dronet(self,image_response):
        ...
        self.gTruthsFile = open(self.base_path + "/images/labels.txt","a")
        self.imgNameFile = open(self.base_path + "/images/imgnames.txt","a")
        ...
        error = self.calculate_covariance()
    
        cv2.imwrite(os.path.join(self.base_path, 'images','images-0', "frame000" + str(self.iteration).zfill(len(str(self.num_samples))) + '.png'), image_noisy)
        self.gTruthsFile.write(str(error) + "\n")
        self.imgNameFile.write("frame000" + str(self.iteration) + ".png" + "\n")

        self.imgNameFile.close()
        self.gTruthsFile.close()
