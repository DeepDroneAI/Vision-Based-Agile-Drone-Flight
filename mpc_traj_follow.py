...
class MPC_controller:
    def __init__(self):
        ...
        self.ref_pose=np.zeros(4)
        self.mpc=None
        self.x0=np.zeros(9)
        self.cost_param_pose=1
        self.cost_param_vel=0
        self.cost_param_yaw=1
        self.cost_param_yaw_rate=0
    ...
    def __template_model(self,symvar_type='SX'):
        ...
        cost=self.cost_param_pose*np.sqrt(pow(self.ref_pose[0]-Pose[0],2)+
                                        pow(self.ref_pose[1]-Pose[1],2)+
                                        pow(self.ref_pose[2]-Pose[2],2))+self.cost_param_vel*np.sqrt(
                                        pow(self.ref_vel[0]-Velocity[0],2)+
                                        pow(self.ref_vel[1]-Velocity[1],2)+
                                        pow(self.ref_vel[2]-Velocity[2],2))+self.cost_param_yaw*np.sqrt(
                                        pow(self.ref_pose[3]-Attitude[2],2))+self.cost_param_yaw_rate*np.sqrt(
                                        pow(self.ref_vel[3]-Rate[2],2))
    ...
    def __set_model(self,x,traj):
        self.ref_vel=np.array(traj[1])
        self.ref_pose=np.array(traj[0])
    ...
    def run_controller(self,x,traj):
        self.__set_model(x=x,traj=traj)
